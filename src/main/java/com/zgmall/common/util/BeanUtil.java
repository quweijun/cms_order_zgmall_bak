...
//import com.neo.galaxy.core.pojo.BaseObject;
...
public abstract class BeanUtil extends BeanUtils {
	...
	public static <T> T bindRequestToBean(HttpServletRequest request, HttpServletResponse response, Class<T> clazz)
	{
		T t = BeanUtil.instantiate(clazz);
		return bindRequestToBean(request, response, t);
	}
	
	public static <T> T bindRequestToBean(HttpServletRequest request, HttpServletResponse response, T t)
	{
	 ...
	 * @return
	 */
	public static <T> T mapToBean(Map map, T obj)
	{
		...
		for(PropertyDescriptor targetProperty : pds)
		{
			...
			if(map.containsKey(key) && targetProperty.getWriteMethod()!=null )
			{
				try
				{
					...
					throw new FatalBeanException("Could not copy property ["+key+"] from source to target.", ex);
				}
			}
		}
		return obj;
     ...
     *  throws an exception
     */
    public static void copyGmProperties(Object dest, Object orig)
